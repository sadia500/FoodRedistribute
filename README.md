...
- **`npm run test:rules`** — ownership, role, verification, and audit-log enforcement in `firestore.rules` (self-escalation, cross-owner writes, admin-only actions, suspended accounts, self-verification, audit immutability — 52 cases), run against the real Firestore emulator via `@firebase/rules-unit-testing`. Needs the emulator, which `firebase emulators:exec` downloads on first run — that requires normal internet access to `storage.googleapis.com` (not available in every sandboxed environment, which is why this suite was originally written without being executable where it was built). It has since been run for real on a machine with normal internet access, which caught and led to the fix of a genuine rules bug (`myRole()` used `&&` where Firestore Rules requires both sides to be boolean — see `IMPLEMENTATION_PLAN.md` §6–7 for the full story).
